...
def check_omit_make_command(path_dict, output, result, threads=30):
    ...
    with open(outputfile, 'w') as outfile:
        outfile.write('timeout_duration=18000\n')
        ...
        for family in family_list:
            ...
            for j, sample in enumerate(path_dict[family]):
                ...
                if relation < 2:
                    ...
                    etching_check_file = f'{etching_output_dir}{etching_prefix}.scored.filtered.typed.vcf'
                    if not os.path.exists(etching_output_dir):
                        os.makedirs(etching_output_dir)

                    etching_cmd = f'timeout 18000 {etching} -1 {trim_files[0]} -2 {trim_files[1]} -g {genome} -f {pgkp_db} -t {threads} --keep-kmc -o {etching_prefix} --output-dir {etching_output_dir} --work-dir {etching_output_dir}'
                ...
                if j == len(path_dict[family]) -1:
                    ...
                    if not os.path.exists(f'{pgkp_db}.kmc_suf'):
                        kmc_cmd = f'timeout 18000 {kmc} -v -k31 -m12 -cx10000 -ci5 -t{threads} -fq @{parentfile} {family_dir}{parent_db} {family_dir}'
                        outfile.write(f'{kmc_cmd}\n')
                        outfile.write('exit_status=$?\nif [ $exit_status -ne 0 ]; then\n')
                        outfile.write('echo "Warning: timed out or exited with non-zero status. Skipping..."\n')
                        outfile.write('continue\nfi\n\n')
                        
                        parent_db = family_dir + parent_db
                        kmc_union_cmd = f'timeout 10800 {kmc_tools} simple {parent_db} -ci5 {pgk} union {pgkp_db} -ocsum -ci5 '
