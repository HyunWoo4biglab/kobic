...
def check_omit_make_command(path_dict, output, result, filter_code, threads=30):
    """
    ...
    if not output.endswith('/'):
        output += '/'
    outputfile = output + f'getSVStat_currenlyCompleted_240312.sh'
    ...
    with open(outputfile, 'w') as outfile:
        outfile.write(f'printf "sample\\tfamily\\tDEL\\tDUP\\tINV\\tBND\\n" >> {result}\n')
        for family in family_list:
            ...
            for j, sample in enumerate(path_dict[family]):
                ...
                if not path.endswith('/'):
                    path += '/'
                #print('------',family, sample, path)
                files = get_files_by_id(sample, path)
                files.sort()
                trim_files = list(map(lambda x: trim_dir + re.sub('f.*q.gz', 'trim.fastq', x), files))
                if '-x' in trim_files[0]:
                    underbar_files = list(map(lambda x: re.sub('-x', '_x', x), trim_files))
                elif '_x' in trim_files[0]:
                    underbar_files = list(map(lambda x: re.sub('_x', '-x', x), trim_files))
                gz_files = list(map(lambda x: re.sub('trim.fastq', 'trim.fastq.gz', x), trim_files))
                ...
                if (not os.path.exists(trim_dir)) or (not os.path.exists(trim_files[0])):
                    if not os.path.exists(underbar_files[0]):
                        #print(gz_files[0])
                        if os.path.exists(gz_files[0]):
                            r1 = gz_files[0].split('.gz')[0]; r2 = gz_files[1].split('.gz')[0]
                            mv_cmd = f'mv {gz_files[0]} {r1}\nmv {gz_files[1]} {r2}\n'
                            #outfile.write(f'{mv_cmd}')
                        else:
                            print(family, trim_dir, trim_files[0])
                            #outfile.write(f'{sickle_cmd}\n')
                if relation < 2:
                    parentfile_list.extend(trim_files)
                else:
                    etching_prefix = f'{family}_sample_{sample}'
                    etching_check_file = f'{etching_dir}/{etching_prefix}/{etching_prefix}.scored.filtered.typed.vcf'
                    ...
                    etching_cmd_d[sample] = {'exist' : os.path.exists(etching_check_file), 'cmd' : etching_cmd, 'checkfile' : etching_check_file}
                ...
                if j == len(path_dict[family]) -1:
                    ...
                    if not os.path.exists(f'{pgkp_db}.kmc_suf'):
                        ...
                        kmc_union_cmd = f'{kmc_tools} simple {parent_db} -ci5 {pgk} union {pgkp_db} -ocsum -ci5 '
                        #outfile.write(f'{kmc_union_cmd}\n')

                    for s in etching_cmd_d:
                        if etching_cmd_d[s]['exist']:
                            targetfile = etching_cmd_d[s]['checkfile']
                            filtoutput = targetfile.split('.vcf')[0] + '.PASS_50bp_contigFilt.vcf'
                            filt_cmd = f'python {filter_code} -i {targetfile} -c 50 -s etching -o {filtoutput}'
                            outfile.write(f'{filt_cmd}\n')
                            get_result(f'{filtoutput}', s, family, outfile, result)
...
def get_result(inputfile, sample, family, outfile, result):
    ...
    outfile.write(f'printf "{sample}\\t{family}\\t')
    outfile.write('${del}\\t${dup}\\t${inv}\\t${bnd}\\n" >> ')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f' '--filtercode', help = 'python code to filter VCF', required = True)
    args = parser.parse_args()


    path_dict = get_meta_data(args.input_ped)
    check_omit_make_command(path_dict, args.output, args.sv_result, args.filtercode, args.num_threads)
