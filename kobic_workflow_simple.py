...
def get_meta_data(metafile):
    ...
    with open(metafile) as infile:
        lines = [line for line in infile if not line.startswith('family')]
        for line in lines:
            col = line.strip().split()
            ####
            familyid, sampleid, path, relation = col ### change this with the given metadata format
            ####
            if not familyid in path_dict:
                path_dict[familyid] = dict()
            path_dict[familyid][sampleid] = (relation, path)
...
def make_command(path_dict, interval, output, threads=30):
    ...
    with open(outputfile, 'w') as outfile:
        for family in family_list[start : end]:
            family_dir = working_dir + f'family_{family}/'
            ...
            parentfile = family_dir + f'family_{family}_sample.list'
            parent_db = f'family_{family}_parent'
            pgkp_db = family_dir + f'family_{family}_pgkp'
            ...
            for j, sample in enumerate(path_dict[family]):
                ...
                if relation in ('father', 'mother'):
                    ...
                    etching_prefix = f'family_{family}_sample_{sample}'
